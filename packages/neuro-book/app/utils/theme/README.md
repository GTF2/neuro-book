...
`app/utils/theme/chrome-level.ts` 定义两个档位：`full`（默认，与未引入档位时完全一致）与 `quiet`。

解决的问题：界面里「线」和「面」是两种分层手段。默认档两者都开着，于是每个面板都是「底色 + 描边」，面板一多界面就只剩下线。`quiet` 档收线、并让导航 chrome 退场。

`quiet` 档只动三样，都是「收掉装饰、保留功能」：

| 变量 | 取值 | 为什么 |
|---|---|---|
| `--border-color` | 本主题线色保留 55%，其余退向 `transparent` | 收掉装饰性的缝 |
| `--bg-sidebar` | 本主题的 `--bg-main` | 活动栏退到 app 底色 |
| `--panel-surface` | 本主题的 `--bg-main` | 左侧工具面板退场，稿面成为唯一亮面 |

**不动** `--border-strong`（承载 hover / focus / 可拖拽边界这类**功能性**边界，退场会让人找不到可操作区域），也**不动** `--editor-bg` / `--bg-panel`（前者是稿面本身，后者被 541 处消费，弹层/卡片/对话面都在用）。

**右侧 Agent 面板刻意不退**：它虽然也是面板，但是内容面——你在里面读回复、写指令，和稿面同属"纸"。两边都退会得到对称噪声，反而分不出主次。

两条实测结论，别改回去：

- 线的派生**必须基于该主题自己的线色**，不能「从文字色取固定比例」——各主题线色相对文字色的深浅本来就不一样，取固定比例会让一部分主题的线**变深**（实测 light 下取 20% 把 `#e5e7eb` 加重到了 `#cfd1d4`）。
- 线**退向 `transparent`**，不要退向某个具体底色——线坐在活动栏、面板、主区各种底上，写死底色会在别的底上跑偏（试过退向 `--bg-panel`，线在灰色活动栏上距底色只剩 1，等于消失）。

**覆盖必须做在变量解析阶段**（`resolveChromeVars`），不能写成 CSS 规则：36 个颜色变量由 `apply-theme.ts` 写在宿主元素的 **inline style** 上，优先级高于任何选择器。写成 `.novel-ide-theme[data-chrome="quiet"] { … }` 会被静默压掉，现象是「档位切了但界面没变」。

档位额外写入的角色映射 token（`--panel-surface`）登记在 `chromeOverriddenTokens`，`applyThemeVars` 会连同主题变量一起清理——不然从「简洁」切回「标准」时内联值残留，侧栏会一直退场且看不出原因。

入口：设置 → 前端设定 → 界面观感。人眼验证也可用 `?chrome=quiet`（临时覆盖，不写回存储）。
