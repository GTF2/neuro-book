...
describe("World Engine Workbench preview redesign", () => {
    it("保留 mock 数据源和三栏 preview 入口", async () => {
        ...
        expect(sliceList).toContain("只看仍有待处理问题的切片");
        expect(sliceList).toContain("只看已审查处理完成的切片");
        expect(sliceList).toContain("只看没有派生问题的干净切片");
        ...
        expect(sliceCard).toContain("元数据草稿");
        expect(sliceCard).toContain("未应用元数据草稿");
        ...
        expect(sliceCard).toContain("值草稿 {{ props.valueDraftCount }}");
        expect(sliceCard).toContain("未应用值草稿");
        ...
        expect(sliceCard).toContain("文件 {{ subjectFileProposalCount }}");
        ...
        expect(inspector).toContain("仅生成建议，不会自动写入");
        ...
        expect(patchEditor).toContain("补丁草稿");
        ...
        expect(editor).toContain("同一主体 + 属性路径");
