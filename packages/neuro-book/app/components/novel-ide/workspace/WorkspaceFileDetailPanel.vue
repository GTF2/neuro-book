...
<template>
    ...
    >
        ...
        <template #actions>
            <span v-if="isDirty" class="inline-block h-1.5 w-1.5 rounded-full bg-[var(--status-warning)]" :title="t('ide.workspace.common.unsaved')"></span>
            ...
            <button v-if="draft" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-[10px] text-[var(--accent-text)] hover:bg-[var(--bg-hover)] disabled:cursor-not-allowed disabled:opacity-45" type="button" :disabled="!isDirty || savingFile" @click="void saveDraft({notify: true})"><span v-if="savingFile" class="i-lucide-loader-circle h-3 w-3 animate-spin"></span>{{ t("ide.workspace.common.save") }}</button>
        </template>
        ...
        <div v-if="props.node" class="grid min-w-0 gap-2 text-xs text-[var(--text-secondary)]">
            ...
            <!-- 表单化条目面板：对齐世界书面板形态（028 统一） -->
            <div v-if="draft" class="min-w-0 space-y-2">
                <div class="space-y-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-2">
                ...
                <div class="space-y-2">
                    <div class="space-y-1">
                        ...
                        <TagInput :model-value="draft.aliases" :placeholder="t('ide.workspace.common.addAlias')" @update:model-value="draft.aliases = $event; void saveDraft()" />
                    </div>
                    <div class="space-y-1">
                        ...
                        <TagInput :model-value="draft.tags" :placeholder="t('ide.workspace.common.addTag')" accentStyle @update:model-value="draft.tags = $event; void saveDraft()" />
                    </div>
                    <div class="space-y-1">
                ...
                <!-- manuscript 卷/章统计卡片（026 单位版保留） -->
                <div v-if="isManuscriptIndexFile" class="space-y-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-2">
