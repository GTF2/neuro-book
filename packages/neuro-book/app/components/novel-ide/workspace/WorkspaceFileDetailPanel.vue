...
<template>
    ...
    >
        ...
        <template #actions>
            <span v-if="isDirty" class="text-[10px] text-[var(--status-warning)]">{{ t("ide.workspace.common.unsaved") }}</span>
            ...
            <button v-if="draft" class="rounded-md px-2 py-1 text-[10px] text-[var(--accent-text)] hover:bg-[var(--bg-hover)] disabled:cursor-not-allowed disabled:opacity-45" type="button" :disabled="!isDirty || savingFile" @click="void saveDraft()">{{ t("ide.workspace.common.save") }}</button>
        </template>
