        ...
        <div class="flex min-h-0 flex-1 flex-col space-y-4 pt-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <div class="flex flex-wrap items-center gap-1 rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] py-1 pl-1.5 pr-1">
                    <span class="px-1 text-[11px] font-semibold text-[var(--text-secondary)]">{{ t("agent.session.status") }}</span>
                    <button v-for="item in statusItems" :key="`status-${item.value}`" type="button" class="rounded-md border px-2 py-1 text-[11px] transition-colors" :class="statusFilter === item.value ? 'border-[var(--accent-main)] bg-[var(--accent-bg)] text-[var(--accent-text)]' : 'border-transparent bg-transparent text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]'" @click="statusFilter = item.value">{{ item.label }}</button>
                </div>
                <div class="flex flex-wrap items-center gap-1 rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] py-1 pl-1.5 pr-1">
                    <span class="px-1 text-[11px] font-semibold text-[var(--text-secondary)]">{{ t("agent.session.profile") }}</span>
                    <button v-for="item in profileItems" :key="`profile-${item.value}`" type="button" class="rounded-md border px-2 py-1 text-[11px] transition-colors" :class="profileFilter === item.value ? 'border-[var(--accent-main)] bg-[var(--accent-bg)] text-[var(--accent-text)]' : 'border-transparent bg-transparent text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]'" @click="profileFilter = item.value">{{ item.label }}</button>
                </div>
                <div class="flex flex-wrap items-center gap-1 rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] py-1 pl-1.5 pr-1">
                    <span class="px-1 text-[11px] font-semibold text-[var(--text-secondary)]">{{ t("agent.session.relation") }}</span>
                    <button v-for="item in relationItems" :key="`relation-${item.value}`" type="button" class="rounded-md border px-2 py-1 text-[11px] transition-colors" :class="relationFilter === item.value ? 'border-[var(--accent-main)] bg-[var(--accent-bg)] text-[var(--accent-text)]' : 'border-transparent bg-transparent text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]'" @click="relationFilter = item.value">{{ item.label }}</button>
                </div>
                <button type="button" class="ml-auto inline-flex h-8 items-center gap-1 rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-2.5 text-[11px] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]" @click="resetFilters">
