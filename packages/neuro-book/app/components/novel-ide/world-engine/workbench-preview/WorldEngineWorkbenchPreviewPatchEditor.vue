    ...
    <div class="flex min-h-0 flex-1 flex-col">
        <div class="mb-2 flex shrink-0 flex-wrap items-center justify-between gap-2">
            <div class="flex min-w-0 items-center gap-2">
                ...
                <span v-if="props.dirty" class="rounded border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-1.5 py-0.5 text-[10px] font-semibold text-[var(--we-warning)]">补丁草稿</span>
            </div>
            ...
            >
                ...
                <label class="flex h-7 min-w-0 items-center gap-2 rounded border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-2 transition-colors focus-within:border-[var(--we-accent-border)] focus-within:bg-[var(--we-bg-panel)]">
                    <span class="flex shrink-0 items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-[var(--we-text-muted)]">
                        <span>摘要</span>
                        <span v-if="row.dirty" class="rounded border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-1 py-0 text-[9px] normal-case tracking-normal text-[var(--we-warning)]">未保存</span>
                        <span v-if="row.highlighted" class="rounded border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-1 py-0 text-[9px] normal-case tracking-normal text-[var(--we-warning)]">问题目标</span>
                    </span>
                    <input :value="row.mutation.summary ?? ''" class="h-full w-full min-w-0 bg-transparent text-[11px] text-[var(--we-text-main)] outline-none disabled:opacity-60" placeholder="本次补丁的说明" :disabled="props.busy" @input="emit('update-summary', row.index, inputValue($event))">
                </label>
            </article>
