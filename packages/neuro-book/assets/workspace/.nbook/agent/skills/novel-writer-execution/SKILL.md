...
- **result**：已写入路径、润色情况、约 100 字剧情总结，**末尾追加「## 本章结算」块**（格式见下）。
- **data**：默认不填；除非调用方明确需要结构化结果。
- **不输出**：写作分析、草稿过程或自查清单。

「## 本章结算」块（写后结算表）：对已写正文的如实事实清点，供 leader 写后评审直接消费。固定格式：
...
要点：三个小节必须齐全，没有内容的小节写「- 无」；结算块是**写完之后的事后上报**，不是写作前置条件，不要在动笔前准备它，也不要为凑结算内容改写正文；结算块只出现在 `report_result.result` 里，不写进正文文件。
...
write 正文
  ↓
report_result（result 末尾附「## 本章结算」块）
```
...
此时必须在 `report_result.result` 里明确点出"本轮新增 / 改动了哪些尚未登记到 World Engine 的角色或状态"，交给 leader 事后补回——这些内容同时按规范进「## 本章结算」块的新增事实小节，不要另起一套格式。
...
- 剧情点全部覆盖。
- 角色视角与信息边界没有越界。
- 已通过 `report_result` 报告写入路径与剧情摘要，result 末尾附「## 本章结算」块（三节齐全）。
