...
> 本环节的写-评-修可以整体交给 `run_workflow` 的 `chapter-write-review-revise` 编排：它调用真实 writer 写入目标章节文件，三个评审维度（一致性/节奏/文风）并发挑问题，writer 按 major 问题修订循环。args 传 `chapterPath` + `chapterId`（都必填：writer 按 chapterId 自取事实简报）+ `brief`（可选，**只注入评审**，不下发 writer）+ `infoControl`（**通常不必手填**：宿主装配了只读查询 `plot.chapter-info-control@1` 时，workflow 会按 `chapterId` 自动从 ChapterBrief 的 `readerKnows` / `protagonistKnows` / `mustHide` / `hintOnly` 四字段编译成「读者已知 / 主角已知 / 必须隐藏 / 可暗示」清单，返回值 `infoControlSource=auto`；需要覆盖时才显式传入（`provided`）。宿主未装配该查询时退回手填，**自动编译不可用又未手填也不会静默跳过**：一致性评审显式标注「信息边界未核对」，返回值 `infoControlSource=missing`、`infoControlChecked=false`——看到这个标记就补上清单重跑。清单**只注入一致性评审**，不下发 writer）+ `lorebookEntries` / `reviewRounds`(1-3) / `revise`。前置与手动流程相同：剧情事实已拍板、World Engine 已推进、章节节点已存在。需要逐步人工把关或用户要参与每轮决策时，仍按本文手动循环。轻量非章节文本（简介、文案）用 `write-review-loop`（不写文件）。
>
...
6. 如果产生新事实，记录后续 World Engine 回补事项；用户确认后再写入切面。产生新事实的章节，修订收敛后进入第六步做未来影响分析。
...
## 第六步：未来影响分析（正文采纳后的下游体检）

反向核对（第四步评审、`consistency-audit`）盯的是"正文有没有写错"；本步骤盯的是**另一件事**：本轮正文确认的新事实，有没有让**还没写到的下游计划**失效。原理与清单格式见 `reference/plot/future-impact-analysis.md`。

**何时跑**：修订收敛后、本章收口前，且**本轮产生了新事实**（结果改写、物品易主、角色位置 / 伤势变化、信息披露）。没有新事实就跳过，直接进入完成标准。

**怎么扫**（三类都要扫，全部只读）：

1. **Promise 面**：`get_story_promise` —— 本章挂了哪些 beat；有没有未兑现且期限 / 节奏已过或临近的 Promise；新事实是否让某条承诺的兑现**在物理上不可能**。
2. **Scene 面**：`get_story_thread` / `get_story_scene_context` —— 本章之后同 Thread 的后续 Scene、相邻章 Scene，是否与新事实冲突。
3. **帧面**：`get_story_keyframe` / `get_tween_keyframes` —— 本章是否落在补间区间内；帧声明的不可逆变化是否被正文**推翻**。

**产出**：一份受影响清单（`target` / `impactType` / `evidence` / `suggestedAction`）+ 人读 markdown；按面分组，无受影响项的面显式写"未发现"，不要省略。

**谁裁决**：把清单交给作者逐项确认"改"或"保持"。选"改"的项走既有写入路径（`save_story_scene` / `save_story_promise` / `save_story_keyframe`；推翻帧另配 `save_story_decision` 留痕）。本步骤**只标记，不自动改规划**。

**失败语义**：某面查询失败时，显式标注"该面未完成核对"，绝不把"没查到"说成"没有问题"。
...
- 修订产生的新事实已落库或显式挂起。
- 若本章本轮产生新事实：已完成未来影响分析（第六步），受影响清单已产出并逐项确认（改计划的项走既有写入路径，保持的项视为已裁决）；某面查询失败时已显式标注"未完成核对"，未冒充"未发现"。
