...
**Pending Questions 双通道硬规则**：写入文件只完成一半，必须在同一条回复的对话里主动逐项向用户提出，每项附 2-3 个候选方向供参考或点选。只写文件不在对话里问是违规——用户看不到文件里的待定项，问题会石沉大海，之后设定被随手补成默认值。候选方向是建议不是替答：用户选其中一个、给第五种答案、或明确说"先放着"，都算闭环；没有任何一轮对话向用户提出过，就不算处理完。
