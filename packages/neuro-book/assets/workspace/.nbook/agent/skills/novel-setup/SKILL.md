...
**触发条件**：项目还是新建状态（manuscript 下没有正文、没有 story-concept，`PROJECT-STATUS.md` 缺失或刚初始化），且这是用户在本项目的头一两条消息。老项目、导入、续写都不触发。

满足时，先回应欢迎再进入入口分流，人话、简短、不表单化：
...
3. 顺势进入下面的入口判断，按用户实际输入分流；用户已经一口气把点子说完时，不要为了欢迎而欢迎，直接接住内容进入分流。
