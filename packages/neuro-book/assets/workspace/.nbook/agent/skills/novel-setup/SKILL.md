...
首次欢迎的触发与话术由 `novel-guide` 统一承担（leader 路由层每条消息必经，规则若只挂在本 skill 会在用户被路由到其他 skill 时丢失）。本项目从本 skill 进入且用户尚未给出点子时，遵循 novel-guide 的"首次欢迎"节：一句欢迎+三条示例开场白，再进入下面的入口判断；不要为欢迎而欢迎。
...
- 不确定的设定显式标记为待定（`PROJECT-STATUS.md` Pending Questions 或 `status: pending` 节点），不伪装成已确认事实。写 Pending Questions 必须走双通道：同一轮对话里逐项向用户提出并附候选方向（硬规则见阶段一），只写文件不算处理完。
