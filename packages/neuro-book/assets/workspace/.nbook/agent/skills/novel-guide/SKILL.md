...
**触发条件**：本会话的第一条消息，且用户还没有给出具体点子（开口只是"我想写个故事"这类）。**不需要检查项目产出状态来判断"是否首次"**——拿不准就先欢迎（成本仅一句话）；用户已经说了具体点子则跳过欢迎直接接内容。

思考与回复一律用大白话，不引用技能名/协议名/内部术语（不说"novel-guide skill""创世向导薄版""分流"这类词，用户不需要知道它们）。

满足时，先输出欢迎再按下面的速查分流，人话、简短、不表单化：

1. 一句欢迎，传达"跟我说一句你的点子就行，剩下的我来办"——用户不需要懂任何系统概念，一句话就能开始。
2. 给三条示例开场白，覆盖三种典型起点，告诉用户"像这样说话就行，可以照抄、可以改、也可以完全按自己的来"：
   - 有一句话灵感：「我想写一个末世里开小卖部的故事，大概讲……」
   - 有现成素材：「我手头有一些设定和旧稿，帮我整理成一本能往下写的书。」
   - 完全没想法：「我想写小说但完全没头绪，陪我聊聊，帮我找到一个想写的故事。」
3. 欢迎说完即按用户实际输入分流（点子模糊 → `novel-idea-exploration`；点子完整 → `novel-setup`）；用户已经一口气把点子说完时，不要为了欢迎而欢迎，直接接住内容进入分流。
