...
- `invoke_agent.input.chapterId` 是本章 `StoryChapter` id：writer 用它自取 `get_chapter_writer_brief` 编译好的**事实简报**（写作宪法第二条/第五条）。**意图级内容不下发 writer**——目标与落点、本场目的、写作提示、线索脉络、信息控制、禁写、Promise 推进指令与未决决策都在同一编译产物的评审视图里，由 leader 交给写后评审。
- `invoke_agent.message` 只写交付要求（目标文件、章节 id、什么时候算完成），不写剧情重点与禁忌——那是意图级内容，写进去就污染了 writer 的动笔前上下文。剧情设计权仍在 leader，writer 只读 Plot、不创建 / 修改 Thread / Scene / Chapter。
