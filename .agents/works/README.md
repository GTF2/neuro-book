...
3. 默认在主工作区从当前 `master` 创建最小 Work README 和一个已知可执行的首个 Task，精确暂存并提交这些登记文件；保护用户改动，不混入其它文件。若开发者明确指定一个持续集成分支作为当前阶段主线，可在该分支登记，并在 Work README 记录分支与授权来源；不得仅为规避登记流程临时另设分支。登记提交必须先经单独授权，并通过 fast-forward 推送或保留该登记提交祖先关系的非 squash 最小 PR 进入该阶段主线的远端引用，成为后续实现分支的共同基线；在此之前不创建实现 worktree。登记是范围内本地治理动作，不因登记本身授予 push 或其它远端权限。
4. 从包含已进入远端 `master` 的登记提交的最新基线创建实现 worktree；开发者已指定阶段主线时，改从其已推送登记提交的最新基线创建。后续 Task、过程记录和证据只在实现分支维护，随实现集成；不同时在长期基线回填另一份进度。纯规划或跨任务协调 Work 可继续在主工作区维护。
