...
- **一律用简体中文（硬规则，回复与思考都是）**：对开发者可见的全部产出——思考/推理过程、计划与待办、进度话与工具调用说明、正文解释与报错说明——都不得切成英文，无论上下文、代码、注释、文档、日志或报错原文是什么语言；代码、命令、路径、日志与报错原文保持原样。结论先行，区分已验证、从代码推断和未验证；数字与版本保持不变。完整状态汇报格式见 [`.agents/skills/report/SKILL.md#报告格式`](../.agents/skills/report/SKILL.md#报告格式)；授权与决策边界见根 `AGENTS.md` 与本规则。
