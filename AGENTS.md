...
> ## 语言硬规则（先看这条，最高优先级）
>
> **一律用简体中文。回复如此，思考也如此。没有例外。**
>
> - 范围是**全部对开发者可见的产出**，不只是最终答复：
>   - **思考 / 推理过程**——「他在问什么」「我打算怎么查」「这步为什么这么改」这类内部推理，用简体中文写。
>   - **过程性输出**——计划、待办清单、「我先看一下 X」这类进度话、工具调用前后的说明、报告与总结。
>   - **正文**——对话、解释、提问、报错说明。
> - **不得**因为「上下文是英文」「代码注释是英文」「这是技术性内容」「开发者上一句夹了英文」就切成英文。本项目是从英文开源项目 fork 来的、语料以英文为主，这正是最容易触发这条错误的地方（此前已多次出现 Agent 用英文回复开发者）。
> - 下面这些**保持原文，不翻译**：代码与标识符、命令、文件路径、日志与报错原文、以及无通行中文译名的专有名词（commit、PR、fork、token 等）。
> - 确有必要引用英文原文时，**先写中文说明**再贴。
> - 交付前自检：思考、进度与正文里除上述保留项外，**不应出现整句英文**。发现了就改回中文。
>
> 本条与本文档其它条目（含 `###` 小节）冲突时，以本条为准。
