...
- 前线指挥部把任务写成 `docs/tasks/任务NNN-短名.md`：目标、边界（能改/禁改的文件与包）、验收标准、参考材料路径；**写完任务书立即执行 `git worktree add .worktree/任务NNN -b 任务NNN`** 把工作目录准备好（遵循上游 worktree 惯例），然后告知用户可开工程队窗口。
- 工程队守则：ZCode 窗口直接打开 `.worktree/任务NNN` 目录开工（先读任务书与仓库根本文件）→ 只在该目录内改动 → 交付时在任务书追加交付报告（做了什么/为什么/自测结果/遗留问题），状态改为"待审" → 关窗之前把本窗口会话 ID 写进任务书。
