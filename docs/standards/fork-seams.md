...
| S31 | `server/workspace-files/llmlint-check.ts`（**新增**）、`server/api/workspace-files/llmlint-check.post.ts`（**新增**）、`app/components/novel-ide/NovelProseLintPanel.vue`（**新增**）、`app/components/markdown-studio/{MarkdownStudioToolbar,MarkdownStudioWorkbench}.vue`、`app/i18n/locales/{zh-CN,en-US}.ts` | **llmlint 编辑器入口**：主应用 `app/` 此前对 llmlint 零接入，而 `README.md:114` 宣称它「既是编辑器里的润色 Skill」。runner **spawn CLI** 而非 import——该包 `private: true` 且**没有 `exports`**，公开契约就是「命令行 + 结构化 JSON」。**关键坑**：`check` 的退出码是 eslint 式门禁（有可见 high 命中即 `exit 1`），**不代表执行失败**，故只用「stdout 能否解析出 JSON」判成败。输入是**文件路径 positional**而非 stdin（见 `skill/src/cli.ts` 的 `.argument("<files...>")`）。skill root 按「monorepo 源码副本 → 运行态投影副本」定位，都找不到返回 503。UI 是非模态右侧抽屉。**只做扫描 + 报告，绝不自动改正文**——自动改写属写作流程形态，是写作宪法边界 | 新增 4 文件；改上游 2 个 markdown-studio 组件 + 2 个 i18n | 中（动了上游 markdown-studio 组件与 i18n；上游若自行接入 llmlint 即归零） |
