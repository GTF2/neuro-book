...
| Plot 写回校验清单 | [`plot/write-back-checks.md`](plot/write-back-checks.md) | 写完正文 / canon 后核对什么、判据与失败动作（block/warn/record）的声明式清单，各消费方的单一索引入口；文档级，不引入运行时引擎 |
...
| P1 | 配置、模型与凭据 | `packages/neuro-book/server/config/`、`packages/neuro-book/server/models/`、`packages/neuro-book/shared/dto/app-settings.dto.ts` | 配置优先级、敏感字段、provider identity、错误和 UI 行为没有单一规范；辅助任务模型来源已晋升 `implemented`（[`agent/auxiliary-task-model.md`](agent/auxiliary-task-model.md)），设置面板的保存反馈仍属本缺口 |
