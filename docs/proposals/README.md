...
- [`../packages/neuro-book/docs/proposals/agent-followup-queue-delivery.md`](../../packages/neuro-book/docs/proposals/agent-followup-queue-delivery.md)：Agent durable follow-up 队列的来源可见性、单条与批量处理入口、暂停后的空闲自愈，状态为 `accepted`。
...
- [`stable-release-admission.md`](./stable-release-admission.md)：canary → stable 的准入条件（25 项，分技术门禁 / 发布工艺 / 产品验收三组；公开签名、macOS 公证、后台 updater、真实作者流程验收等 7 项未满足），状态为 `draft`。
