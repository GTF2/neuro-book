...
每一章可以带一组**写作指令**，但它们分两类，走两条完全不同的路（写作宪法第二条 / 第五条）：

- **进 writer 动笔前上下文的**：视角人物、语气，以及章节所属场景的时间 / 地点 / 在场角色与世界状态。
- **只用于写完之后核对的**：本章目标与落点、开头收尾要求、信息控制、禁写事项。

信息控制这组字段（读者知道什么、主角知道什么、必须隐瞒什么、只能暗示什么）是希区柯克悬念理论的工程化——桌下有炸弹而角色不知道，悬念才成立。**AI 天然是上帝视角**，所以要管住它。

但管的方式是**事后校验**：这些字段不会被塞进 writer 动笔前的上下文——塞了它会写得保守、平滑、一股报告味——而是在正文写完后作为核对清单去撞正文，只砍真正越界的地方。四个字段全空也不再阻断写作交接。
