...
如果某一章正好落在两个**关键帧**之间，还有另一个入口：你只声明前后两帧的**不可逆变化**（谁拿到什么、谁死了、哪条誓破了、什么东西从此不能再用），中间的正文交给模型演化；写完再拿帧声明去撞正文，撞出冲突才由你裁决。

- 帧目前用**对话**维护：让 Agent「把这一对帧记下来」，它用 `save_story_keyframe` 落库；也可以让它列出两个帧之间的路标（`get_tween_keyframes`）。
- **专用界面面板还没做**：关键帧面板尚未提供，现在只能通过对话（或直接改项目数据）查看和编辑。
- 帧里写的是事实，不是写作指令；推翻一个帧必须同时留一条创作决策记录。
